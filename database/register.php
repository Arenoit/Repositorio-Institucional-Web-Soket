...
if(!empty($bootuser)&&!empty($name)&&preg_match($regex,$email)&&!empty($password)){
    ...
    elseif(!empty($menssage2['emailExist']))echo 'Correo ya existe';
        ...
        $signin = "INSERT INTO acausers(users_usr_users,users_nom_users,users_eml_users,users_pas_users,users_typ_users,users_hash_users,users_fec_users)  SELECT '$bootuser','$name','$email','$hash',1,'$hashValidate',CURRENT_DATE WHERE NOT EXISTS(SELECT 1 FROM acausers WHERE users_eml_users='$email' AND users_val_users=1)";
